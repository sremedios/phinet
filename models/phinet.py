...
def phinet_2D(n_classes, model_path, num_channels=1, learning_rate=1e-3, num_gpus=1, verbose=1):
    ...
    for _ in range(4):
        ...
        y = Activation('relu')(x)
        x = Conv2D(8, (3,3), strides=(1,1), padding='same')(y)
        x = BatchNormalization()(x)
        x = add([x, y])
        x = Activation('relu')(x)

    # this block will pool a handful of times to get the "big picture" 
    y = MaxPooling2D(pool_size=(5,5), strides=(1,1), padding='same')(inputs)
    y = AveragePooling2D(pool_size=(3,3), strides=(1,1), padding='same')(y)
    y = Conv2D(8, (3,3), strides=(1,1), padding='same')(y)

    # this layer will preserve original signal
    z = Conv2D(8, (3,3), strides=(1,1), padding='same')(inputs)
    z = Conv2D(8, (3,3), strides=(1,1), padding='same')(z)
    z = Conv2D(8, (3,3), strides=(1,1), padding='same')(z)
