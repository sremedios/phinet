...
if __name__ == "__main__":
    ...
    FOLD_RESULTS_FILE = os.path.join(
            RESULTS_DIR, "test_metrics_fold_{}.txt"
    )
    ...
    if len(sys.argv) > 1:
        TEST_TF_RECORD_FILENAME = sys.argv[1]

        FOLD_RESULTS_FILE = os.path.join(
            RESULTS_DIR, 
            os.path.basename(
                TEST_TF_RECORD_FILENAME
            )[:os.path.basename(TEST_TF_RECORD_FILENAME).find(".")] + 
            "_test_metrics_fold_{}.txt"
        )
